...
class SE_ResNeXt():
    ...
    def bottleneck_block(self,
        ...
        return fluid.layers.elementwise_add(x=short, y=scale, act='relu')  #输入 x 与输入 y 逐元素相加，并将各个位置的输出元素保存到返回结果中
    ...
    def squeeze_excitation(self,
        ...
        scale = fluid.layers.elementwise_mul(x=input, y=excitation, axis=0) #输入 x 与输入 y 逐元素相乘，并将各个位置的输出元素保存到返回结果中
        return scale